...
/***/ function(module, exports, __webpack_require__) {
	...
	var _configJs = __webpack_require__(4);
	...
	var _mainRedditJs = __webpack_require__(129);

	var _mainRedditJs2 = _interopRequireDefault(_mainRedditJs);

	var _mainCarrotJs = __webpack_require__(134);

	var _mainCarrotJs2 = _interopRequireDefault(_mainCarrotJs);
	...
	function setup() {
	    ...
	    if ((0, _jquery2['default'])('body').length !== 1) {
	    ...
	    if (window.location.host.match(/reddit\.com/)) {
	        (0, _mainRedditJs2['default'])();
	    } else if (window.location.host.match(_configJs2['default'].carrotUrl.replace('https://', '') // remove  initial https://
	    .replace(/\/$/, '') // remove trailing slash
	    )) {
	            (0, _mainCarrotJs2['default'])();
	        }
	}
	...
	// page
	var windowLocation = decodeURIComponent('' + window.location.href);

	if (windowLocation.indexOf('https://ssl.reddit.com/api/v1/authorize.compact') === 0 && (windowLocation.indexOf('redirect_uri=https://carrot.com') > -1 || windowLocation.indexOf('redirect_uri=https://localhost:5443') > -1)) {
	    (0, _jquery2['default'])('body').css({ display: 'none' });
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _injectChatJs = __webpack_require__(130);

	var _injectChatJs2 = _interopRequireDefault(_injectChatJs);

	var _subredditLogicJs = __webpack_require__(132);

	var _subredditLogicJs2 = _interopRequireDefault(_subredditLogicJs);

	var _reddit__frontPageJs = __webpack_require__(133);

	var _reddit__frontPageJs2 = _interopRequireDefault(_reddit__frontPageJs);
	...
	function mainReddit() {
	  (0, _jquery2['default'])('body').addClass('domain-reddit');
	  ...
	  if (window.location.pathname === '/' || window.location.pathname === '/r/all' || window.location.pathname === '/r/all/') {
	    (0, _reddit__frontPageJs2['default'])();
	  }

	  // Run sub-reddit specific logic
	  (0, _subredditLogicJs2['default'])();

	  // Attempt to inject chatroom for the corresponding subreddit
	  (0, _injectChatJs2['default'])();
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports['default'] = injectChatRoom;
	...
	var _getSubredditNameJs = __webpack_require__(131);

	var _getSubredditNameJs2 = _interopRequireDefault(_getSubredditNameJs);
	...
	function destroyEmbed() {
	    (0, _jquery2['default'])('#carrot-room-wrapper').remove();
	    (0, _jquery2['default'])('.carrot-room-wrapper-div').remove();
	    if ($CARROT_WRAPPER && $CARROT_WRAPPER.remove) {
	        $CARROT_WRAPPER.remove();
	    }
	}
	...
	function createEmbed(subredditName) {
	    ...
	    var carrotUrl = _configJs2['default'].carrotUrl + 'embed/r/' + subredditName;

	    var iconUrl = 'https://carrot-prod-assets.s3.amazonaws.com/icons/' + subredditName.toLowerCase() + '.png';
	    var headerIconsUrl = chrome.extension.getURL('img/extension__header-icons.png');
	    ...
	    requestAnimationFrame(function () {
	        ...
	        setTimeout(function () {
	            requestAnimationFrame(function () {
	                ...
	                if ((0, _jquery2['default'])('#tb-toolbarcounters').children().length > 0) {
	                    (0, _jquery2['default'])('#tb-toolbarcounters').css({ 'margin-right': '330px' });
	                }
	            });
	        }, 2000);
	    });
	...
	function injectChatRoom() {
	    ...
	    chrome.runtime.sendMessage({
	        ...
	        if (!data.isValid) {
	            // Do nothing
	            logger.log('carrot/injectChatRoom:noSubredditExists', 'no subreddit exists');
	            return false;
	        }
	         ...
	         */
	        var firstTimeEmbedWasOpenedAfterPageLoad = true;
	        // starts closed by default
	        var embedIsOpen = false;

	        var embedOpenDate = undefined;
	        var embedHideDate = new Date(); // time before first open starts now

	        var createdDate = new Date();
	        var secondsEmbedWasOpened = null;
	        var secondsBeforeOpen = null;
	        var wasClicked = false;

	        var embedWasManuallyDestroyed = false;
	        ...
	        function visibilityChanged() {
	            logger.log('carrot/injectChatRoom:visibilityChanged', 'visibility changed to: ' + document.visibilityState);
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports['default'] = getSubredditName;
	...
	function getSubredditName() {
	    ...
	    if (windowPath === '/') {
	        ...
	        // grab subreddit name
	        var matched = windowPath.match(/\/r\/([^\/]+)/);
	        if (matched) {
	    ...
	    _lodash2['default'].each(blackList, function (item) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports['default'] = runSubredditLogic;
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports['default'] = frontpageLogic;
	...
	function frontpageLogic() {
	    ...
	    function insertCarrotRooms() {
	        ...
	        chrome.runtime.sendMessage({
	            ...
	            var app = state.app;
	            (0, _jquery2['default'])('#carrot__rooms-wrapper').remove();

	            // if there are no keys (user has no rooms), or if they are not
	            // logged in, then don't render anything
	            if (!app.username || !app.unreadByRoom || Object.keys(app.unreadByRoom).length < 1) {
	                return false;
	            }

	            // prepare divs
	            var carrotRoomsWrapper = (0, _jquery2['default'])('<div id="carrot__rooms-wrapper"></div>');
	            var carrotRooms = (0, _jquery2['default'])('<div id="carrot__rooms-inner"></div>');
	            carrotRooms.append('<div id="carrot__rooms-header">Your Carrot Rooms</div>');

	            // convert data to ordered array
	            var unreads = [];
	            _lodash2['default'].each(app.unreadByRoom, function (val, key) {
	                val.slug = key;
	            ...
	            unreads.sort(function (a, b) {
	                var sortA = a[0].toLowerCase();
	                var sortB = b[0].toLowerCase();
	                if (sortA < sortB) {
	                    return -1;
	                } else if (sortB < sortA) {
	                    return 1;
	            ...
	            unreads.forEach(function (d) {
	                var room = d[1];
	                var roomEl = (0, _jquery2['default'])('<div class="carrot__room-wrapper ' + (room.count > 0 ? 'carrot__room-wrapper--read' : 'carrot__room-wrapper--unread') + '"></div>');
	                ...
	                roomEl.click(function () {
	                    ...
	                    window.open(_configJs2['default'].carrotUrl + 'r/' + room.slug);
	                });
	            ...
	            // Insert rooms
	            if ((0, _jquery2['default'])('.side .sidebox.create').children().length > 1) {
	                (0, _jquery2['default'])('.side .sidebox.create').parent().after(carrotRoomsWrapper);
	            } else {
	                (0, _jquery2['default'])('.side .sidebox.submit.submit-text').parent().after(carrotRoomsWrapper);
	            }
	        });
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	function mainCarrot() {
	    ...
	    (0, _jquery2['default'])('body').addClass('domain-carrot');
	    ...
	    chrome.runtime.onMessage.addListener(function gotMessage(message, sender, sendResponse) {
	        ...
	        switch (message.messageType) {
	            ...
	            default:
	                logger.log('chrome-extension/chrome.onMessage:default', 'No case handled for ' + message.messageType);
	    ...
	    if (window.location.pathname.replace(/\//g, '').toLowerCase() === 'loading') {
	        var checkLoggedInState = function checkLoggedInState() {
	            chrome.runtime.sendMessage({ messageType: 'fetchStoreState' }, function (data) {
	                ...
	                if (data.storeState.username) {
	                    ...
	                    chrome.runtime.sendMessage({ messageType: 'fetchAndUpdateNotifications' }, function () {});

	                    logger.log('chrome-extension/chrome.gotData', 'not logged in yet...store: ', data);
	                    if (numCalls > 4) {
	                        clearInterval(checkInterval);
	                    }
	                }
